...
static void frame(void) {
    ...
    for (int y=0;y<height;y+=192) {
        for (int x=0;x<width;x+=192) {
            ...
            sgp_set_view(0, fb_resolve_texture_view);
...
static void init(void) {
    ...
    fb_resolve_view = sg_make_view(&(sg_view_desc){
        ...
        .label = "fb-resolve-view"
    });
    fb_resolve_texture_view = sg_make_view(&(sg_view_desc){
        .texture.image = fb_resolve_image,
...
static void cleanup(void) {
    ...
    sg_destroy_view(fb_resolve_texture_view);
